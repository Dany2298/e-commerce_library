...
async function renderBooks(filter) {
  ...
  if (filter === "LOW_TO_HIGH") {
    books.sort(
      (
        a,
        b //using or operator to prioritize sale price. if it exists take it, else use the original price
...
setTimeout(() => {
  renderBooks(); //js loads before the dom element does. in order, to fix that we put it in a set timeout.
});

function getBooks() {
  return new Promise((resolve) => {
    //how an api works. in the 1 second it fetches data where we will show the loadingstate.
    setTimeout(() => {
      resolve([
        {
          id: 1,
